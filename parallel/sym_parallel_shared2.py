...
def check_epidemic(node):
        ...
        epidemic_state_s = shared_memory.SharedMemory(name="shmem")
        
        if epidemic_state_s.buf[node] == 'S':
            
            neighbours = np.array(g[node])
            
            if neighbours.shape[0] > 0:
                    
                    i_neighbours = neighbours[epidemic_state_s.buf[neighbours] == 'I']
        ...
        elif epidemic_state_s.buf[node] == 'I':
...
if __name__ == '__main__':
    ...
    shm = shared_memory.SharedMemory(name = "shmem",create=True, size=epidemic_state.nbytes)
    
    epidemic_state_shared = np.ndarray(epidemic_state.shape, dtype=epidemic_state.dtype, buffer=shm.buf)
    epidemic_state_shared[:] = epidemic_state[:]
    
    with ProcessPoolExecutor(max_workers=processes, initializer=init_pool, initargs=(mu,g,lambd)) as executor:
          
        for t in tqdm(range(1, T)):
            
            r = executor.map(check_epidemic, nodes_list)

            epidemic_state_shared[:] = np.array(list(r))[:]
    ...
    df.to_csv(save_path_local + 'si_test.csv')
